...
- Your system must also support GPT boot/UEFI mode. Users have experienced issues with systems that only support MBR boot/Legacy BIOS mode. (If your device is less than 10 years old you shouldn't need to worry about this.)
    - Instructions for checking if your device supports UEFI varies for each device. Check with your device manufacturer for instructions on how to check on your own device.
